...
use crate::state::{BoundGuiState, ProjectSaveState};
...
impl Widget for Timeline {
    ...
    fn on_build(&mut self, state: &mut State, entity: Entity) -> Self::Ret {
        ...
        let controls = ListView::with_template(|item: &TimelineTrackSaveState, _| TrackControls::new(&item.name.clone()))
            // .bind(
            ...
            //     |tracks| tracks.clone(),
            // )
            .bind2(BoundGuiState::save_state.then(ProjectSaveState::timeline_tracks))
        ...
        ListView::with_template(|item: &TimelineTrackSaveState, index: usize| Track::new(item.name.clone(), index))
            // .bind(
            //     AppData::backend_handle
            //         .then(BackendHandle::save_state)
            //         .then(ProjectSaveState::timeline_tracks),
